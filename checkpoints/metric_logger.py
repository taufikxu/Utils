...
class MetricSaver(object):
    ...
    def update(self, step=None, value=None, save=True):
        ...
        if len(self.ma_value) == 1:
            ...
            self.ma_value.append(
                self.ma_value[-1] * self.ma_weight + value * (1.0 - self.ma_weight)
            )
    ...
    def save(self):
        if self._test_valid_step() is True:
            ...
            plt.plot(
                self.step[1:], self.value[1:], "-",
            )
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(
                os.path.join(self.path, self.name + self.postfix), bbox_inches="tight"
            )
            plt.close(fig)
...
def build_logger(folder=None, args=None, logger_name=None):
    ...
    coloredlogs.install(
        level=logging.DEBUG, fmt=FORMAT, datefmt=DATEF, level_styles=LEVEL_STYLES
    )
